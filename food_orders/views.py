import json
from django.shortcuts import render
import requests


def food_order_details(request, month):
    url = "http://canteen.benzyinfotech.com/api/v3/customer/report"
    token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9..CwDEjlHoRtOXdFcaO6KGGxV202AOA7MMtJVPtKzgLqzTFzUUnDLGBd7PNAtHO2--3YOathM9HOG8hYjY8wjktXZIoCGUR9GWIaEVUxLwFq927CrSf05NuqTBTrJcDeBOjXDvKcSBiJ2A994FC2IunPcdkaZ4jpoaWBIaWueYUbHviYSQuLec3tFcAMg4njrImAlaN9k-QKkHetpdrdbUEX1Wzq4X-1QwuOx7W3W2nbbxaoNgFX1gaabxi00ZO7h5MokGvtqy_gCkS9TYoM74VfxmTyAAczjttLcPqDNiAL_ZJdutDMezw32CZj8G8l8PUL46F_BuaxatZDBUZxeClZh4_0Wvo9GX4zqF2XvHdzZHnwdB414vNCl8itaGW9w7QWbdchPOglhnek32ZmkH0MIqeOBhnAyHo5_WbP0uLd_3qmz3w04nvTbTGV25-QebaxPAsVD0-7Za1sVpqB_FD6yEeliaEzdxl_8gA5IH59uowpfPYgUIjom8NVEASuYsAwb0q3f0jhNRfwg2zmXNenoDunh_dN9l2NRjI2gdZueSMwu6IJLQK46jpn01uG2iQ1xx-pFJAGe_bzSceLsho3dbtabym3tMqi0Ac02xUP9Mn50LdkFJGNVU9jiuHQfyjQirDtGUfya3aIvpJlCGx9Cx99s_4P89uDnOiXy3A1Q"
    data = {"month": month}
    if not month or not (1 <= int(month) <= 12):
        return render(request, 'food_orders/error.html', {'error_message': 'Invalid month'})

    headers = {"Authorization": f"Bearer {token}"}

    response = requests.post(url, json=data, headers=headers)

    if response.status_code == 200:
        json_data = response.json()
        user_data = json_data.get('user', {})
        reports = json_data.get('reports', [])

        total_fine = 0

        for report in reports:
            opt_ins = report.get('opt_ins', {})
            if isinstance(opt_ins, dict):
                for key, value in opt_ins.items():
                    if value == "Pending":
                        total_fine += 100

        return render(request, 'food_orders/food_order_details.html',
                      {'user_data': user_data, 'reports': reports, 'total_fine': total_fine})

    return render(request, 'food_orders/food_order_details.html')
